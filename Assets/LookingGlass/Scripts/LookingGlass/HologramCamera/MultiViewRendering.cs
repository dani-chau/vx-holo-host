...
namespace LookingGlass {
    internal static class MultiViewRendering {
        ...
        private static void RenderView(int viewIndex, ref RenderViewSharedData data, bool persistViewTextures, out RenderTexture viewRT, out RenderTexture viewRTRFloat) {
            ...
            viewRT = RenderTexture.GetTemporary(data.renderSettings.ViewWidth, data.renderSettings.ViewHeight, 24, data.quilt.format);
        ...
        public static void SetLightfieldMaterialSettings(HologramCamera hologramCamera, Material lightfieldMaterial) {
            ...
            lightfieldMaterial.SetFloat(lightfieldProperties.pitch, cal.ProcessedPitch);

            lightfieldMaterial.SetFloat(lightfieldProperties.slope, cal.ProcessedSlope);
            lightfieldMaterial.SetFloat(lightfieldProperties.center, cal.center
#if UNITY_EDITOR
                + hologramCamera.CameraProperties.CenterOffset
#endif
            );
            lightfieldMaterial.SetFloat(lightfieldProperties.subpixelSize, (float) 1 / (3 * cal.screenWidth) * cal.FlipMultiplierX);
...
// #endif
            ...
            DeviceType deviceType = cal.GetDeviceType();
            bool shouldDimEdgeViews =
                (deviceType == DeviceType._32in || deviceType == DeviceType._65in) &&
                !hologramCamera.Preview2D;

            lightfieldMaterial.SetInt(lightfieldProperties.filterEdge, shouldDimEdgeViews ? 1 : 0);
        }
    }
}
